...
LONG ApplicationCrashHandler(EXCEPTION_POINTERS *pException)
{
	...
	string info;
	//info += crashinfo.ErrorCode;
	//info += crashinfo.Address;
	//info += crashinfo.Flags;
	...
	for (vector<CallStackInfo>::iterator i = arrCallStackInfo.begin(); i != arrCallStackInfo.end(); ++i)
	{
		...
		info += callstackinfo.MethodName;
		info += "() : [";
		info += callstackinfo.ModuleName;
		info += "] (File: ";
		info += callstackinfo.FileName;
		info += " @Line ";
		info += callstackinfo.LineNumber;
		info += ")";
